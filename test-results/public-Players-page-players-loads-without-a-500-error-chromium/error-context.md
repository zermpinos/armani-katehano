...
- Name: public.spec.js >> Players page (/players) >> loads without a 500 error
- Location: e2e/public.spec.js:99:3

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:3000/players
Call log:
  - navigating to "http://localhost:3000/players", waiting until "load"
...
> 100 |     const response = await page.goto("/players");
  ...
  133 |     await page.goto("/");
  134 |     await page.getByRole("link", { name: /games/i }).click();
  135 |     await expect(page).toHaveURL(/\/games/);
  136 |   });
  137 | 
  138 |   test("clicking Leaderboard nav link navigates to /leaderboard", async ({ page }) => {
  139 |     await page.goto("/");
  140 |     await page.getByRole("link", { name: /leaderboard/i }).click();
  141 |     await expect(page).toHaveURL(/\/leaderboard/);
  142 |   });
  143 | });
  144 | 
```