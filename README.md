...
- **Maintenance** page for the global maintenance-mode toggle, the playoff-popup control (enable/disable, semifinal/final round), an aggregate recompute, and a test alert that reports whether import notifications reach Slack or email.
...
- `team-schedule.ts` parses the club's team pages, whose URLs are stored on `League.listingUrl` so a league that starts mid-season needs no redeploy, and `discover-games.ts` splits what it finds: rows with a posted result become import candidates, rows without become fixtures. This is where candidates come from: a game's URL does not exist until the fixture is published, so it cannot be recorded on an `UpcomingGame` row in advance. The same fetch carries the schedule, so `sync-fixtures.ts` writes it to `UpcomingGame` at no extra request. Games are keyed by the id in the URL, never the URL itself, since the same game has been served under both `/winter-cup/` and `/super-winter-cup/`.
...
- The admin's Maintenance page can send a test alert through that same path and reports which channel took it. Because the fallback means a misnamed webhook still delivers by email, that report is the only way to tell a working webhook from a silently unused one.
