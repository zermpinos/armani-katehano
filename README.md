...
- The scheduled fixture also supplies the `round`. A box score cannot say whether a game was a playoff, so `commitImport` reads it off the matching `UpcomingGame`: tag the fixture as `semifinal` or `final` when you schedule it and the imported game inherits that. A game imported from a URL with no fixture is always `regular`.
...
- `/api/cron/poll-imports` - daily at 21:00 UTC (Vercel cron). Scrapes the source URL of up to 3 scheduled games that tipped off between 7 days and 90 minutes ago, and commits the ones that come back complete and internally consistent. Anything else is skipped and stays in the admin's quick-import list. The window is a week rather than a day so a game blocked on someone adding a player or configuring a season still imports itself once they have. Each run records its per-URL outcome in `CronRun.summary`, and emails `ADMIN_ALERT_EMAIL` about any skip that will not resolve on a later run, plus once on the final run that will see a game still stuck for a self-resolving reason. A game that is merely still being played, or that was already imported, is never alerted on.
