export type Snap = { pts: number; fg2m: number; fg2a: number; fg3m: number; fg3a: number; ftm: number; fta: number };

// players: jersey number -> stats snapshot (only non-zero scorers)
export type GameFixture = { url: string; akScore: number; players: Record<number, Snap> };
