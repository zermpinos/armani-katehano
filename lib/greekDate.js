...
export function parseGreekDate(str) {
  ...
  // Strip optional leading weekday token, e.g. "Σάββατο, 28 Μαρτίου 2026"
  // A weekday token contains a comma or is not a digit -- skip it.
  const allParts = str.trim().split(/\s+/);
  const parts = allParts[0].includes(',') || isNaN(parseInt(allParts[0], 10))
    ? allParts.slice(1)
    : allParts;
