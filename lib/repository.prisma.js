...
// ─── Shared helper: merged aggregate rows -> statsMap ─────────────────────────
// Used by getStats() and getAllSeasonsStats() so the shaping logic lives once.

function aggregatesToStatsMap(aggregates) {
  ...
  for (const [pid, agg] of Object.entries(merged)) {
    ...
    // The stored tsPct column is per-league; after merging leagues we recompute
    // from ptsTotal / (2 * (fgaTotal + 0.44 * ftaTotal)).
