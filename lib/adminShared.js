...
// ─────────────────────────────────────────────────────────────────────────────
export function AdminLayout({ slug, children, title = "Admin", toast, setToast, onLogout }) {
  ...
  const logout = async () => {
    ...
    if (onLogout) {
      onLogout();           // let the page handle state (dashboard case)
    } else {
      router.push(`/admin/${slug}`);  // sub-pages redirect to root
    }
  };
